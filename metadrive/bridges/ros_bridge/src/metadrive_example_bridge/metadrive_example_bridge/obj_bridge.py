...
class ObjectPublisher(Node):
	...
	def timer_callback(self):
		...
		for obj_info in obj_infos:
			obj_info = obj_info.tolist() # converto in lista python
			size = Vector3(x=obj_info[3], y=obj_info[4], z=obj_info[5]) # lunghezza, larghezza, altezza
			pos = Point(x=obj_info[0], y=obj_info[1], z=0.0) # posizione relativa al veicolo
			rot = Quaternion(x=0.0, y=0.0, z=np.sin(obj_info[2] / 2.0), w=np.cos(obj_info[2] / 2.0))
			pose = Pose(position=pos, orientation=rot) # combina posizione e orientamento
			bbox = BoundingBox3D(center=pose, size=size) # creo il boundingbox che poi verrà pubblicato sul nodo ROS2
			bboxes.append(bbox)
